...
class Agentscraper:
    ...
    def retry_request(self, url, max_retries=5, retry_delay=3):
        ...
        return None
    def parse(self,product_selector,absolut_url):
            ...
            xpaths = [
                '//h1[.="About"]/following-sibling::div[2]/p/text()',
                '//div[@style="text-align: center;"]/span/text()',
                '//*[@class="rng-agent-profile-content"]/div[2]/text()',
                '//div[@style="text-align: center;"]/text()',
                '//*[@class="rng-agent-profile-content"]/div/p/text()',
                '//*[@class="rng-agent-profile-content"]/descendant::p/descendant::span/text()',
                '//*[@class="rng-agent-profile-content"]/descendant::strong/text()',
                '//*[@class="rng-agent-profile-content"]/descendant::span[2]/text()',
                '//*[@class="rng-agent-profile-content"]/descendant::li/text()',
                '//*[@class="rng-agent-profile-content"]/div[2]/p/text()',
                # Add other XPath expressions as needed
            ]
            ...
            for xpath in xpaths:
                result = product_selector.xpath(xpath).getall()
                if result and any(desc.strip() for desc in result):
                    description = [desc.strip() for desc in result if desc.strip()]
                    break 
