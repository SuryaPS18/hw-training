...
def parse(api_url):
    ...
    locality = location_list[-1].get('name')
    ...
    for license in licenses:
        ...
        if authority == 'DED':
            ...
            rera_length = len(rera_regis)       
    dtcm_license=ded_license
    broker = data.get('agency', {}).get('name')
    broker_display_name = broker.upper()
    ...
    listing_id = data.get('externalID')  
    ...
    iteration_number=1
    listing_availability = data.get('state')
    if listing_availability == 'active':
        listing_availability = True
    else:
        listing_availability = False
    details=data.get('area')
    extracted_data={
